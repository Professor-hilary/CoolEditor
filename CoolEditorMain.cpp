...
CoolEditorFrame::CoolEditorFrame( wxWindow* parent, wxWindowID id ) {
    ...
    Create( parent, wxID_ANY, _( "CoolEditor" ), wxDefaultPosition, wxDefaultSize, wxDEFAULT_FRAME_STYLE, _T( "wxID_ANY" ) );
    ...
    AuiManager1->AddPane( AuiNotebookConsolePane, wxAuiPaneInfo().Name( _T( "ConsolePane" ) ).DefaultPane().Caption( _( "Console" ) ).CaptionVisible().MinimizeButton().Position( 1 ).Bottom().DockFixed().BestSize( wxSize( 0, 100 ) ) );
    ...
    AuiManager1->AddPane( AuiNotebookManagement, wxAuiPaneInfo().Name( _T( "LeftPane" ) ).DefaultPane().Caption( _( "Management" ) ).CaptionVisible().Layer( 1 ).Left().BestSize( wxSize( 150, 0 ) ) );
    ...
    AuiManager1->AddPane( AuiNotebookEditors, wxAuiPaneInfo().Name( _T( "MainViewPane" ) ).DefaultPane().Caption( _( "Editors" ) ).CaptionVisible().MinimizeButton().MaximizeButton().CloseButton( false ).Center().DockFixed().DestroyOnClose() );
    ...
    AuiManager1->AddPane( AuiNotebookAside, wxAuiPaneInfo().Name( _T( "Aside" ) ).DefaultPane().Caption( _( "Side Pane" ) ).CaptionVisible().MinimizeButton().MaximizeButton().PinButton().CloseButton( false ).Right().BestSize( wxSize( 150, 0 ) ) );
    AuiManager1->Update();
    ...
    //  stc->SetBackgroundColour( wxColour( 37, 44, 46 ) );
...
void CoolEditorFrame::setCppStyling() {
    ...
    // Default text color
    stc->StyleSetForeground(wxSTC_STYLE_DEFAULT, wxColour(255, 255, 255));
    stc->StyleSetBackground(wxSTC_STYLE_DEFAULT, backgroundColour);
    ...
    stc->StyleSetForeground(wxSTC_C_COMMENTLINE, wxColour(0, 128, 0));       // Green
    stc->StyleSetBackground(wxSTC_C_COMMENTLINE, backgroundColour);
    stc->StyleSetItalic(wxSTC_C_COMMENTLINE, true);
    stc->StyleSetForeground(wxSTC_C_COMMENTDOC, wxColour(0, 128, 0));        // Green
    ...
    // Numbers
    stc->StyleSetForeground(wxSTC_C_NUMBER, wxColour(255, 0, 255));          // Magenta
    stc->StyleSetBackground(wxSTC_C_NUMBER, backgroundColour);

    // Keywords
    stc->StyleSetForeground(wxSTC_C_WORD, wxColour(0, 128, 255));            // Blue
    ...
    // Operators
    stc->StyleSetForeground(wxSTC_C_OPERATOR, wxColour(255, 255, 0));        // Yellow
